...
//			VehicleHomeLoan vh1 = new VehicleHomeLoan(LoanType.Vehicleloan, 500000, 4.3f, LocalDate.of(2022, 03, 03),
...
//					"9865345021");
//			Address a3 = new Address(2023103l, AddressType.HomeAddress,"ABC Apartment", "2nd Cross", "Banglore", "Karnataka", "414001");
...
//*******************************************READING CUSTOMERS FROM TXT FILE*******************************************
		...
		try {
			...
			while (fis.available() > 0) {
				...
				Customer c = new Customer(cs.getCustId(), cs.getName(), cs.getDOBasString(), cs.getEmploymentstatus(),
						cs.getEmail(), cs.getPhonenumber());
				try {
					// Bank bank = new Bank();
					bank.customers.put(cs.getCustId(), c);
				} catch (Exception e) {
					e.printStackTrace();
				}
				// System.out.println(cs); //PRINTING LIST
			}
			ois.close();
			fis.close();
		} catch (IOException | ClassCastException | ClassNotFoundException e) {
			e.printStackTrace();
		} catch (IllegalDOBException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		} catch (IllegalEmailIDException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		} catch (IllegalPhoneNumberException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}

//*******************************************READING CUSTOMERS ADDRESS FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
			...
			while (fis.available() > 0) {
				...
				Address address = new Address(a.getCustID(), a.getAddressType(), a.getBuildingname(),
						a.getStreetAddress(), a.getCity(), a.getState(), a.getZipCode());
				try {
					// Bank bank = new Bank();
					// System.out.println(a);
					// bank.customers_address.get(a.getCustID()).put(a.getAddressType(), address);
					// bank.addCustomerAddress(a.getCustID(),a.getAddressType(), address);
					if (bank.customers_address.containsKey(a.getCustID())) {
						bank.customers_address.get(a.getCustID()).put(a.getAddressType(), address);
						// StoreIntoTxt.storeCutomersAddress(customers_address);
					} else {
						bank.customers_address.put(a.getCustID(), new HashMap<AddressType, Address>());
						bank.customers_address.get(a.getCustID()).put(a.getAddressType(), address);
						// StoreIntoTxt.storeCutomersAddress(customers_address);
					}
				} catch (Exception e) {
...
//*******************************************READING CLOSED ACCOUNT FROM TXT FILE*******************************************
//		try {
//			FileInputStream fis = new FileInputStream("C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\ClosedAccount.txt");
//			ObjectInputStream ois = new ObjectInputStream(fis);
//			while (fis.available() > 0) {
//				ClosedAccount ca = (ClosedAccount) ois.readObject();
//				//System.out.println(ca); //PRINTING LIST
//				//ClosedAccount c = new ClosedAccount(ca.getCustID(),ca.getAccountno(), ca.getAccountType(), ca.getClosingDateTime());
////				try {
////					if (bank.closed_accounthistory.containsKey(ca.getAccountno())) {
////						bank.closed_accounthistory.get(ca.getAccountno()).put(ca.getClosingDateTime(), c); 
////					} else {
////						bank.closed_accounthistory.put(ca.getAccountno(), new TreeMap<LocalDateTime, ClosedAccount>());
////						bank.closed_accounthistory.get(ca.getAccountno()).put(ca.getClosingDateTime(), c);
////					}
////				} catch (Exception e) {
////					e.printStackTrace();
...
//*******************************************READING SAVINGS ACCOUNT FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
					"C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\SavingsAccount.txt");
			ObjectInputStream ois = new ObjectInputStream(fis);
			while (fis.available() > 0) {
				HashMap<Long, HashMap<String, SavingsAccount>> acc_savings = (HashMap<Long, HashMap<String, SavingsAccount>>) ois
						.readObject();
				// System.out.println(acc_savings); //PRINTING LIST
				for (Map.Entry<Long, HashMap<String, SavingsAccount>> entry : acc_savings.entrySet()) {
					// System.out.println("Key = " + entry.getKey() + ", Value = " +
					// entry.getValue());
					long CustID = entry.getKey();
					for (Map.Entry<String, SavingsAccount> entry1 : entry.getValue().entrySet()) {
						// System.out.println("Key = " + entry1.getKey() + ", Value = " +
						// entry1.getValue());
						String accountno = entry1.getKey();
						SavingsAccount savaccount;
						try {
							savaccount = new SavingsAccount(entry1.getValue().accounttype,
									entry1.getValue().getBalance(), entry1.getValue().getDateofOpen());
							if (bank.accounts_Mainsavings.containsKey(CustID)) {
								bank.accounts_Mainsavings.get(CustID).put(accountno, savaccount); // main savings
																									// account list
							} else {
								bank.accounts_Mainsavings.put(CustID, new HashMap<String, SavingsAccount>());
								bank.accounts_Mainsavings.get(CustID).put(accountno, savaccount);
							}
						} catch (IllegalDepositException e) {
							e.printStackTrace();
						}
...
//*******************************************READING CURRENT ACCOUNT FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
					"C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\CurrentAccount.txt");
			ObjectInputStream ois = new ObjectInputStream(fis);
			while (fis.available() > 0) {
				HashMap<Long, HashMap<String, CurrentAccount>> acc_curr = (HashMap<Long, HashMap<String, CurrentAccount>>) ois
						.readObject();
				// System.out.println(acc_savings); //PRINTING LIST
				for (Map.Entry<Long, HashMap<String, CurrentAccount>> entry : acc_curr.entrySet()) {
					// System.out.println("Key = " + entry.getKey() + ", Value = " +
					// entry.getValue());
					long CustID = entry.getKey();
					for (Map.Entry<String, CurrentAccount> entry1 : entry.getValue().entrySet()) {
						// System.out.println("Key = " + entry1.getKey() + ", Value = " +
						// entry1.getValue());
						String accountno = entry1.getKey();
						CurrentAccount curraccount;
						try {
							curraccount = new CurrentAccount(entry1.getValue().accounttype,
									entry1.getValue().getBalance(), entry1.getValue().getDateofOpen());
							if (bank.accounts_Maincurrent.containsKey(CustID)) {
								bank.accounts_Maincurrent.get(CustID).put(accountno, curraccount); // main savings
																									// account list
							} else {
								bank.accounts_Maincurrent.put(CustID, new HashMap<String, CurrentAccount>());
								bank.accounts_Maincurrent.get(CustID).put(accountno, curraccount);
							}
						} catch (IllegalDepositException e) {
							e.printStackTrace();
						}
		...
		// UNCOMMENT TILL HERE REST HAVE PROBLEM
//*******************************************READING FD ACCOUNT FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
					"C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\FDAccount.txt");
			ObjectInputStream ois = new ObjectInputStream(fis);
			while (fis.available() > 0) {
				HashMap<Long, HashMap<String, FixedDepositAccount>> acc_fd = (HashMap<Long, HashMap<String, FixedDepositAccount>>) ois
						.readObject();
				// System.out.println(acc_savings); //PRINTING LIST
				for (Map.Entry<Long, HashMap<String, FixedDepositAccount>> entry : acc_fd.entrySet()) {
					// System.out.println("Key = " + entry.getKey() + ", Value = " +
					// entry.getValue());
					long CustID = entry.getKey();
					for (Map.Entry<String, FixedDepositAccount> entry1 : entry.getValue().entrySet()) {
						// System.out.println("Key = " + entry1.getKey() + ", Value = " +
						// entry1.getValue());
						String accountno = entry1.getKey();
						FixedDepositAccount fdaccount;
						try {
							fdaccount = new FixedDepositAccount(entry1.getValue().accounttype,
									entry1.getValue().getInvestment(), entry1.getValue().getInterest(),
									entry1.getValue().getCurrDate(), entry1.getValue().getcompoundfrequency(),
									entry1.getValue().getyears());
							if (bank.accounts_Mainfixeddeposit.containsKey(CustID)) {
								bank.accounts_Mainfixeddeposit.get(CustID).put(accountno, fdaccount); // main savings
																										// account list
							} else {
								bank.accounts_Mainfixeddeposit.put(CustID, new HashMap<String, FixedDepositAccount>());
								bank.accounts_Mainfixeddeposit.get(CustID).put(accountno, fdaccount);
							}
						} catch (IllegalDepositException e) {
							e.printStackTrace();
						}
...
//*******************************************READING LOAN ACCOUNT FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream("C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\Loan.txt");
			ObjectInputStream ois = new ObjectInputStream(fis);
			while (fis.available() > 0) {
				HashMap<Long, HashMap<String, VehicleHomeLoan>> acc_curr = (HashMap<Long, HashMap<String, VehicleHomeLoan>>) ois
						.readObject();
				// System.out.println(acc_savings); //PRINTING LIST
				for (Map.Entry<Long, HashMap<String, VehicleHomeLoan>> entry : acc_curr.entrySet()) {
					// System.out.println("Key = " + entry.getKey() + ", Value = " +
					// entry.getValue());
					long CustID = entry.getKey();
					for (Map.Entry<String, VehicleHomeLoan> entry1 : entry.getValue().entrySet()) {
						// System.out.println("Key = " + entry1.getKey() + ", Value = " +
						// entry1.getValue());
						String accountno = entry1.getKey();
						VehicleHomeLoan curraccount;
						try {
							curraccount = new VehicleHomeLoan(entry1.getValue().getLoanType(),
									entry1.getValue().getLoan_amount(), entry1.getValue().getInterest(),
									entry1.getValue().getCurrDate(), entry1.getValue().getCompound_frequency(),
									entry1.getValue().getYears(), entry1.getValue().getage());
							if (bank.loan_Mainloan.containsKey(CustID)) {
								bank.loan_Mainloan.get(CustID).put(accountno, curraccount); // main savings account list
							} else {
								bank.loan_Mainloan.put(CustID, new HashMap<String, VehicleHomeLoan>());
								bank.loan_Mainloan.get(CustID).put(accountno, curraccount);
							}
						} catch (IllegalAgeException e) {
							e.printStackTrace();
						}
...
//*******************************************READING DEPOSITS TO SAVINGS FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
					"C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\depositToSavings.txt");
			ObjectInputStream ois = new ObjectInputStream(fis);
			// while (fis.available() > 0) {
			Deposit deposit = (Deposit) ois.readObject();
			bank.accounts_Mainsavings.get(deposit.getCustID()).get(deposit.getAccono()).deposit(deposit.getAmmount());
			// bank.depositToSavings(deposit.getCustID(),deposit.getAccono(),deposit.getAmmount());
			// }
			ois.close();
			fis.close();
		} catch (IOException | ClassCastException | ClassNotFoundException e) {
...
//*******************************************READING DEPOSITS TO CURRENT FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
			...
			// while (fis.available() > 0) {
			Deposit deposit = (Deposit) ois.readObject();
			bank.accounts_Maincurrent.get(deposit.getCustID()).get(deposit.getAccono()).deposit(deposit.getAmmount());
			// bank.depositToSavings(deposit.getCustID(),deposit.getAccono(),deposit.getAmmount());
			// }
			ois.close();
			fis.close();
...
//*******************************************READING WITHDRAW FROM SAVINGS FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
			...
			// while (fis.available() > 0) {
			Deposit deposit = (Deposit) ois.readObject();
			bank.accounts_Mainsavings.get(deposit.getCustID()).get(deposit.getAccono()).withdraw(deposit.getAmmount());
			// bank.depositToSavings(deposit.getCustID(),deposit.getAccono(),deposit.getAmmount());
			// }
			ois.close();
			fis.close();
...
//*******************************************READING WITHDRAW FROM CURRENT FROM TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
			...
			// while (fis.available() > 0) {
			Deposit deposit = (Deposit) ois.readObject();
			bank.accounts_Maincurrent.get(deposit.getCustID()).get(deposit.getAccono()).withdraw(deposit.getAmmount());
			// bank.depositToSavings(deposit.getCustID(),deposit.getAccono(),deposit.getAmmount());
			// }
			ois.close();
			fis.close();
...
//*******************************************READING TRANSACITON RECORDS TXT FILE*******************************************
		try {
			FileInputStream fis = new FileInputStream(
					"C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\RecordedTransaction.txt");
			ObjectInputStream ois = new ObjectInputStream(fis);
			HashMap<String, TreeMap<LocalDateTime, TransactionHistory>> mapFromFile = new HashMap<>();
			// while (fis.available() > 0) {
			// HashMap<String, TreeMap<LocalDateTime, TransactionHistory>> record =
			// (HashMap<String, TreeMap<LocalDateTime, TransactionHistory>>)
			// ois.readObject();
			// System.out.println(record); //PRINTING LIST
			while (fis.available() > 0) {
				String accountno = (String) ois.readObject();
				TreeMap<LocalDateTime, TransactionHistory> value = (TreeMap<LocalDateTime, TransactionHistory>) ois.readObject();
				mapFromFile.put(accountno, value);
				// System.out.println(mapFromFile);
			}

			for (Map.Entry<String, TreeMap<LocalDateTime, TransactionHistory>> entry : mapFromFile.entrySet()) {
				// System.out.println("Key = " + entry.getKey() + ", Value = "
				// +entry.getValue());
				String accountno = entry.getKey();
				for (Map.Entry<LocalDateTime, TransactionHistory> entry1 : entry.getValue().entrySet()) {
					// System.out.println("Key = " + entry1.getKey() + ", Value = "
					// +entry1.getValue());
					LocalDateTime datetime = entry1.getKey();
					TransactionHistory transactionhistory;
					transactionhistory = new TransactionHistory(entry1.getValue().getDatetime(),
							entry1.getValue().getDescription(), entry1.getValue().getAction(),
							entry1.getValue().getAmount(), entry1.getValue().getBalance());
					if (bank.recorded__transactionhistory.containsKey(accountno)) {
						bank.recorded__transactionhistory.get(accountno).put(datetime, transactionhistory);
					} else {
						bank.recorded__transactionhistory.put(accountno,
								new TreeMap<LocalDateTime, TransactionHistory>());
						bank.recorded__transactionhistory.get(accountno).put(datetime, transactionhistory);
					}
				}
			}
...
//*******************************************READING CLOSED LOANS FROM TXT FILE*******************************************

//		try {
//			FileInputStream fis = new FileInputStream("C:\\Users\\asus\\OneDrive\\Desktop\\mybanktxt-dup\\ClosedLoan.txt");
//			ObjectInputStream ois = new ObjectInputStream(fis);
//			while (fis.available() > 0) {
//				ClosedLoan ca = (ClosedLoan) ois.readObject();
//				//System.out.println(ca); //PRINTING LIST
//				//ClosedLoan c = new ClosedLoan(ca.getCustID(),ca.getAccountno(), ca.getAccountType(), ca.getClosingDateTime());
////				try {
////					if (bank.closed_loanhistory.containsKey(ca.getAccountno())) {
////						bank.closed_loanhistory.get(ca.getAccountno()).put(ca.getClosingDateTime(), c); 
////					} else {
////						bank.closed_loanhistory.put(ca.getAccountno(), new TreeMap<LocalDateTime, ClosedAccount>());
////						bank.closed_loanhistory.get(ca.getAccountno()).put(ca.getClosingDateTime(), c);
////					}
////				} catch (Exception e) {
////					e.printStackTrace();
				...
				case 1: // New customer ----------------------------------------------> Main case1
					...
					if (bank.customers.containsKey(CustID)) {
						...
						try {
							...
							Address address = new Address(CustID, AddressType.HomeAddress, buildingname, streetAddress,
									city, state, zipCode);
							...
							if (empstat.equals(EmploymentStatus.Employee)) {
								...
								Address workaddress = new Address(CustID, AddressType.WorkAddress, workbuildingname,
										workstreetAddress, workcity, workstate, workzipCode);
								bank.addCustomerAddress(CustID, AddressType.WorkAddress, workaddress);
								StoreIntoTxt.storeCutomersAddress(workaddress);
							}
					...
					do {
						...
						case 2: // add new account---------------------> Main case2 -> case2
							...
							case 1: // add new savings account---------------------> Main case2 -> case2 -> case1
								...
								if(bank.closed_accountS.containsKey(custid)) {
									System.out.println("re-issue closed account number : "+bank.closed_accountS.get(custid));
								}
								...
								if (bank.customers.containsKey(custid)) {
									...
									if (bank.closed_accountS.get(custid).equals(accno)) {
										bank.closed_accountS.remove(custid);
									}
									...
									try {
										...
										// StoreIntoTxt.storeSavingsAccount(custid, accno, s);
										System.out.println();
							...
							case 2: // add new current account---------------------> Main case2 -> case2 -> case2
								...
								if(bank.closed_accountS.containsKey(custid2)) {
									System.out.println("re-issue closed account number : "+bank.closed_accountS.get(custid2));
								}
								...
								if (bank.customers.containsKey(custid2)) {
									...
									if (bank.closed_accountS.get(custid2).equals(accno1)) {
										bank.closed_accountS.remove(custid2);
									}
...
//added newly
										...
										System.out.println("New Balance: " + currency.getSymbol()
												+ bank.accounts_Mainsavings.get(custid5).get(accono2).getBalance());
							...
							if (bank.customers.containsKey(custid7)) {
								...
								do {
									...
									case 1: // Edit name -------------------------------------------> Main case2 ->
										...
										bank.getCustomerDetails(custid7);
										try {
											Customer editCustomer = new Customer(custid7, new_name,
													bank.customers.get(custid7).getDOBasString(),
													bank.customers.get(custid7).getEmploymentstatus(),
													bank.customers.get(custid7).getEmail(),
													bank.customers.get(custid7).getPhonenumber());
											StoreIntoTxt.editCutomersEditedName(editCustomer);
										} catch (IllegalDOBException e1) {
											// TODO Auto-generated catch block
											e1.printStackTrace();
										} catch (IllegalEmailIDException e1) {
									...
									case 2: // Edit address -------------------------------------------> Main case2 ->
										...
										if (edit == 1) {
											...
											Address new_homeaddress = new Address(custid7, AddressType.HomeAddress,
													new_buildingname, new_streetAddress, new_city, new_state,
													new_zipCode);
											...
											Address new_workaddress = new Address(custid7, AddressType.WorkAddress,
													new_workbuildingname, new_workstreetAddress, new_workcity,
													new_workstate, new_workzipCode);
									...
									case 3: // Edit email -------------------------------------------> Main case2 ->
										...
										try {
											Customer editCustomer = new Customer(custid7, bank.customers.get(custid7).getName(),
													bank.customers.get(custid7).getDOBasString(),
													bank.customers.get(custid7).getEmploymentstatus(),
													new_email,
													bank.customers.get(custid7).getPhonenumber());
											StoreIntoTxt.editCutomersEditedEmail(editCustomer);
										} catch (IllegalDOBException e1) {
											// TODO Auto-generated catch block
											e1.printStackTrace();
										} catch (IllegalEmailIDException e1) {
									...
									case 4: // Edit phone no -------------------------------------------> Main case2 ->
										...
										try {
											Customer editCustomer = new Customer(custid7, bank.customers.get(custid7).getName(),
													...
													bank.customers.get(custid7).getEmail(),
													new_phoneno);
											StoreIntoTxt.editCutomersEditedPhone(editCustomer);
										} catch (IllegalDOBException e1) {
											// TODO Auto-generated catch block
											e1.printStackTrace();
										} catch (IllegalEmailIDException e1) {
									...
									case 5: // Edit Employment Status -------------------------------------------> Main
										...
										try {
											System.out.print(
													"New Employment Status (Worker/Employee/SelfEmployed/UnEmployed): ");
										...
										if (new_empstat.equals(EmploymentStatus.Employee)) {
											...
											Address workaddress = new Address(custid7, AddressType.WorkAddress,
													new_workbuildingname, new_workstreetAddress, new_workcity,
													new_workstate, new_workzipCode);
						...
						case 13: // Close/remove account -------------------------------------------> Main case2
							...
							case 1: // close savings---------------------> Main case2 -> case13 -> case1
								...
								if (bank.accounts_Mainsavings.containsKey(custid4)
									...
									bank.closed_accountS.put(custid4, accono1);
							...
							case 2: // close current---------------------> Main case2 -> case13 -> case2
								...
								if (bank.accounts_Maincurrent.containsKey(custid4)
									...
									System.out.print("Enter Account No: ");
									String accono1 = sc.nextLine();
									bank.closed_accountC.put(custid4, accono1);
				...
				case 3: // Bank log----------------------------------------------> Main case3
					...
					do {
						...
						case 7:// ------------------------------------------> Main case3 -> case7
							...
							switch (choice19) {
							case 1: // all closed accounts(savings/current/fd)---------------------> Main case3 ->
									// case7 -> case1
								System.out.printf("\n%11s %16s %15s %25s", "CUSTOMER ID", "ACCOUNT NUMBER",
										"ACCOUNT TYPE", "DATE & TIME");
								System.out.println();
								bank.getAllClosedAccount();
								break;

							case 2: // all closed loan---------------------> Main case3 -> case7 -> case2
								System.out.printf("\n%11s %16s %15s %15s %25s", "CUSTOMER ID", "ACCOUNT NUMBER",
										"ACCOUNT TYPE", "LOAN AMOUNT", "DATE & TIME");
								System.out.println();
